...
/* Custom style for banner text legibility */
.banner-caption-bg {
...
const bannerData = [
    {
        ...
        description: "Discover the finest organic products sourced directly from nature."
    },
    {
        id: 2,
        image: banner2,
        title: "Fresh & Sustainable",
        description: "Support local farmers and enjoy the freshest seasonal picks."
    },
    {
        id: 3,
        image: banner3,
    ...
    },
    {
        ...
        image: banner4,
        title: "Fast Delivery",
        description: "Order now and get your groceries delivered to your doorstep."
    }
];
...
const renderStars = (rating) => {
    ...
    return Array.from({ length: 5 }, (_, i) => (
        <FontAwesomeIcon
            key={i}
            icon={faStar}
            className={`me-1 transition-colors duration-300 ${
                i < Math.round(ratingValue) ? 'text-yellow-400' : 'text-gray-300'
            }`}
        />
    ));
};
...
function ProductList({ onLoginClick }) {
    ...
    // Effect to ensure Bootstrap Carousel auto-play works
    useEffect(() => {
    ...
    const filteredProducts = products.filter((product) => {
        ...
        const categoryMatch =
            filters.selectedCategory === 'All' || product.category_name === filters.selectedCategory;
        const vendorMatch =
    ...
    if (products.length > 0 && filteredProducts.length === 0)
        return (
            <div className="container text-center py-5 bg-green-50 border border-green-200" style={{ color: THEME_COLOR }}>
                ...
                <p>Try adjusting your category, store, or price range.</p>
            </div>
        );
        ...
        <div className="container py-4 px-2 px-md-4 px-lg-5">
            ...
            <div className="row mb-4">
                <div className="col-12">
                    <div
                        id="bannerCarousel"
                        className="carousel slide carousel-fade rounded-lg shadow-lg overflow-hidden"
                    ...
                    >
                        ...
                        <button className="carousel-control-prev" type="button" data-bs-target="#bannerCarousel" data-bs-slide="prev">
                            <span className="carousel-control-prev-icon bg-dark rounded-circle p-3" style={{ backgroundSize: '50%' }}></span>
                        </button>
                        <button className="carousel-control-next" type="button" data-bs-target="#bannerCarousel" data-bs-slide="next">
                            <span className="carousel-control-next-icon bg-dark rounded-circle p-3" style={{ backgroundSize: '50%' }}></span>
                        </button>
                    </div>
                </div>
            </div>
                        ...
                        {currentProducts.map((product) => {
                            ...
                            return (
                                <div key={product.id} className="col-6 col-sm-6 col-md-4 col-lg-3">
                                    <div className="card h-100 bg-transparent rounded-lg overflow-hidden position-relative border-0 group transition-all">
                                        {discountPercent > 0 && <div className="position-absolute top-0 start-0 m-2 text-white small fw-bold px-2 py-1 rounded-pill z-10" style={{ backgroundColor: THEME_COLOR }}>{discountPercent}% OFF</div>}
                                        <img src={product.image_url || 'https://via.placeholder.com/400x300?text=NO+IMAGE'} alt={product.name} className="card-img-top object-cover transition-transform group-hover:scale-105" style={{ height: '160px' }} />
                                        <div className="card-body d-flex flex-column p-3 align-items-center">
                                            ...
                                            <div className="d-flex justify-content-center align-items-center mt-auto gap-2">
                                                <Link to={`/product/${product.id}`} className="btn btn-sm" style={{ backgroundColor: 'transparent', color: THEME_COLOR, borderColor: THEME_COLOR }} onMouseOver={(e) => { e.currentTarget.style.backgroundColor = THEME_COLOR; e.currentTarget.style.color = 'white'; }} onMouseOut={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = THEME_COLOR; }}>View Details</Link>
                                                <button className="btn btn-sm" style={{ backgroundColor: THEME_COLOR, color: 'white', borderColor: THEME_COLOR }} onMouseOver={(e) => e.currentTarget.style.backgroundColor = THEME_COLOR_HOVER} onMouseOut={(e) => e.currentTarget.style.backgroundColor = THEME_COLOR} onClick={() => handleAddToCart(product.id)} title="Add to Cart"><FontAwesomeIcon icon={faShoppingCart} /></button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
