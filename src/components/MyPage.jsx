...
    faToggleOn, faToggleOff, faEye, faEyeSlash, faUsers, faBan, faHourglassHalf
...
useEffect(() => {
        if (activeView) {
            ...
            // Only navigate if the URL doesn't match the current state
...
                fetchAdminDashboard(),
                fetchAllVendors(),
                fetchAllCustomers(), // ✅ Fetch Customers
                fetchAllProducts(),
                fetchAllOrders(),
                fetchCategories(),
                fetchAdminPayouts(),
...
    
    // ✅ MODIFIED: Approve/Reject/Pending Vendor Logic
...
            message = `Are you sure you want to REJECT and DELETE vendor ${id}?`;
            variant = "danger";
        } else {
            message = `Are you sure you want to set vendor ${id} to PENDING?`;
            variant = "secondary"; // For Pending
        }
...
            try { 
                await authFetch(`${ADMIN_APPROVE_VENDOR_URL}${id}/`, { method: "PATCH", body: JSON.stringify({ action }) }); 
                fetchAllVendors(); 
            } catch (err) { 
...
  const toggleProductPublish = async (id, currentStatus) => {
    ...
    try {
        ...
        // ✅ CORRECT: Re-fetch all products immediately after a successful update.
        // This forces the component using allProducts to re-render with fresh data.
        fetchAllProducts(); 
    } catch (err) {
        // This alerts the user if the request failed or the JSON parsing failed.
        alert(`Error updating publish status: ${err.message}`);
    } finally {
        // Stop the loading spinner (set to null) regardless of success or failure.
...
    // --------------------------------------------------
    
    // ... (Vendor API Functions omitted for brevity, assume same as before) ...
...
    // ✅ MODIFIED: Updated Vendor Management with new Actions and Status Column
    // ✅ MODIFIED: Updated Vendor Management to disable buttons based on status
...
                                if (v.is_approved === true) { statusLabel = "Approved"; statusClass = "bg-green-100 text-green-800"; }
                                else if (v.is_approved === false) { statusLabel = "Pending"; statusClass = "bg-yellow-100 text-yellow-800"; }
                                
                                return (
...
                                                        v.is_approved 
                                                        ? 'bg-gray-200 text-gray-400 cursor-not-allowed opacity-50' // Disabled Style
                                                        : 'bg-green-100 text-green-600 hover:bg-green-200' // Active Style
                                                    }`} 
                                                    title={v.is_approved ? "Already Approved" : "Approve Vendor"} 
                                                    disabled={v.is_approved} // Logic disable
                                                    onClick={() => approveVendor(v.id, "APPROVE")}
                                                >
                                                    <FontAwesomeIcon icon={faCheck} />
                                                </button>

                                                {/* ✅ Pending Button - Disabled if already Pending */}
                                                <button 
                                                    className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200 ${
                                                        !v.is_approved 
...
                                                    }`} 
                                                    title={!v.is_approved ? "Already Pending" : "Set Pending"} 
...
                                                {/* Reject Button - Always active to allow banning */}
                                                <button 
...
        );
    };
    // ✅ NEW: Customer Management Report
    // ✅ UPDATED: Customer Management Report with Debugging
    const renderAdminCustomerMgmt = () => {
        // DEBUGGING: Check what data is actually here
        console.log("Current Customer Data:", allCustomers);

        // Safety check: Ensure allCustomers is an array before filtering
        const safeCustomers = Array.isArray(allCustomers) ? allCustomers : [];

        const filteredCustomers = safeCustomers.filter(c => {
            const searchLower = searchTerm.toLowerCase();
            // Handle cases where username or email might be null/undefined
...
                            
                            {/* Show Empty State */}
                            {!adminLoading && filteredCustomers.length === 0 && ( 
                                <tr><td colSpan="5" className="p-4 text-center text-gray-500">
                                    No customers found. (Check Console for data)
                                </td></tr> 
                            )}
...
                                    <td className="px-6 py-4 whitespace-nowrap">
                                        {/* Check if 'role' exists in data, otherwise assume Customer */}
...
                                        alt={p.name} 
                                        className="w-20 h-20 rounded-md object-cover border border-gray-200 flex-shrink-0 bg-gray-50"
                                    />
...
                                    {/* ✅ NEW: Set Pending Button (Only show if not already pending) */}
                                    {p.status !== 'PENDING' && (
                                        <button className="p-2 w-10 h-10 rounded-full text-yellow-600 hover:bg-yellow-50 transition-colors disabled:opacity-50" title="Set as Pending" disabled={isThisProductUpdating} onClick={() => updateProductStatus(p.id, 'PENDING')} >
                                            {isThisProductUpdating ? <FontAwesomeIcon icon={faSpinner} className="animate-spin"/> : <FontAwesomeIcon icon={faHourglassHalf} />}
                                        </button>
                                    )}

                                    {/* Reject Button (Only show if not already rejected) */}
                                    {p.status !== 'REJECTED' && (
                                        <button className="p-2 w-10 h-10 rounded-full text-orange-600 hover:bg-orange-50 transition-colors disabled:opacity-50" title="Reject" disabled={isThisProductUpdating} onClick={() => updateProductStatus(p.id, 'REJECTED')} >
...
   const renderAdminCategoryMgmt = () => (
        <div className="max-w-4xl">
            ...
            {/* ADD FORM ... (Keep your existing form code here) */}
            <form onSubmit={createCategory} className="flex gap-3 mb-8 bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-inner">
                <div className="flex-grow relative">
                    <input type="text" className="block w-full rounded-lg border-gray-300 pl-4 py-3" placeholder="Type new category name..." value={newCategory} onChange={(e) => setNewCategory(e.target.value)} required disabled={categoryLoading} />
                </div>
                <ThemeButton type="submit" disabled={categoryLoading} className="w-32 shadow-lg">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {categories.map(c => {
                    ...
                    // --- DEBUGGING LOGIC ---
                    // Inside renderAdminCategoryMgmt -> categories.map loop

const getImageSource = (cat) => {
    // 1. Get the raw path from backend
    const rawPath = cat.image || cat.image_url || cat.icon;
    
    if (!rawPath) return null;

    // 2. If it is ALREADY a full URL (starts with http), use it as is.
    if (rawPath.startsWith('http') || rawPath.startsWith('https')) {
        return rawPath;
    }

    // 3. ✅ NEW: Check if it is a Cloudinary path (starts with 'image/upload')
    // We prepend YOUR specific Cloudinary domain
    if (rawPath.startsWith('image/upload')) {
        return `https://res.cloudinary.com/dykdiqcye/${rawPath}`;
    }

    // 4. Fallback: If it's not Cloudinary, assume it's a local Django media file
    const cleanPath = rawPath.startsWith('/') ? rawPath : `/${rawPath}`;
    return `http://127.0.0.1:8000${cleanPath}`;
};

                    const finalImageSrc = getImageSource(c);
                    // -----------------------
                        ...
                        <div key={c.id} className={`relative bg-white p-3 rounded-xl border transition-all duration-300 flex items-center ${isEditing ? `ring-2 ring-[${OLIVE_THEME.main}]` : 'hover:shadow-md border-gray-100'}`}>
                            {isEditing ? (
                                ...
                                <div className="flex items-center gap-2 w-full">
                                    ...
                                    <button onClick={cancelEditingCategory} className="w-8 h-8 rounded-full bg-gray-100 text-gray-500"><FontAwesomeIcon icon={faTimes} /></button>
                                </div>
                            ) : (
                                // VIEW MODE
                                <div className="flex justify-between items-center w-full">
                                        ...
                                        <div className="flex-shrink-0">
                                            {finalImageSrc ? (
                                                <img 
                                                    ...
                                                    onError={(e) => {
                                                        console.error("Image failed to load:", finalImageSrc);
                                            ...
                                            {/* Fallback Letter Circle (Shows if no image OR if image fails to load) */}
                                            <div 
                                                className="w-12 h-12 rounded-lg bg-gray-100 flex items-center justify-center text-gray-400 font-bold text-lg border border-gray-200"
                                    ...
                                    <div className="flex gap-2 ml-2">
                                        <button className="w-9 h-9 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100" onClick={() => startEditingCategory(c)}><FontAwesomeIcon icon={faPencilAlt} /></button>
                                        <button className="w-9 h-9 rounded-full bg-red-50 text-red-600 hover:bg-red-100" onClick={() => deleteCategory(c.id)}><FontAwesomeIcon icon={faTrashAlt} /></button>
                                    </div>
                                </div>
                            )}
                        </div>
...
                            
                            {/* ✅ NEW NAV LINK: Customer Report */}
                            {/* Placed under Vendor Report as requested */}
